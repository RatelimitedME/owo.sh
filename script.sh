...
function shorten() {
	...
	if [[ "$url" =~ $regex ]]; then
		result=$(curl -s "https://api.ratelimited.me/api/shorten/polr?action=shorten&key=$key&url=$url" -H "User-Agent: WhatsThisClient (https://github.com/whats-this/owo.sh, v0.0.19)")
...
function screenshot() {
	...
	entry="$path$filename"
	upload=$(curl -s -F "files[]=@"$entry";type=image/png" https://api.ratelimited.me/api/upload/pomf?key="$key"  -H "User-Agent: WhatsThisClient (https://github.com/whats-this/owo.sh, v0.0.19)")
...
function upload() {
	...
	mimetype=$(file -b --mime-type "$entry")
		...
		if [[ "$filesize" -le 83886081 ]]; then
			upload=$(curl -s -F "files[]=@"$entry";type=$mimetype" https://api.ratelimited.me/api/upload/pomf?key="$key" -H "User-Agent: WhatsThisClient (https://github.com/whats-this/owo.sh, v0.0.19)")
			item="$(grep -E -o '"url":\s*"[^"]+"' <<<"${upload}" | cut -d "\"" -f 4)"
		else
